...
const AddPlace = ({ setMessage, toggleNotifyModal, currentPage, toggleAddPlaceModal, open }) => {
  ...
  const savePlace = () => {
    ...
    if (name.length > 0 && type.length > 0 && position) {
      dispatch(CreatePlace({ data: { name, type, phoneNumber, source: "web", position } }))
        .then(() => {
            ...
            <ListGroupItem>
              <FormGroup>
                ...
                <>
                  ...
                  <Input onChange={(e) => setPhoneNumber(e.target.value)} type="text" name="phoneNumber" id="exampleEmail" />
                </>
              </FormGroup>
            </ListGroupItem>
          </ListGroup>
